...
win.title('AUTOMATIC TELLER MACHINE-ATM')
...
# Initialize global balance variable
def withdrawal_func():
    ...
    # Number buttons (1-9)
    for i in range(1, 10):
        button = Button(bf1 if i < 4 else bf2 if i < 7 else bf3, text=str(i), font=cour15, command=lambda i=i: money_entry.insert('end', str(i)))
    ...
    enter_btn = Button(bf, text='ENTER', font=cour15, fg='red', command=lambda: question_func(money_entry.get()))
    enter_btn.pack(side=LEFT, pady=10)

    clear_btn = Button(bf, text='CLEAR', font=cour15, fg='red', command=lambda: money_entry.delete(0, END))
    clear_btn.pack(side=LEFT, padx=10)

def question_func(amount_str):
    ...
    try:
        ...
        global_balance -= amount  # Deduct the amount from the balance
        balance_func()  # Show balance after withdrawal
    except ValueError:
...
def balance_func():
    ...
    message = Message(balance_win, text=f'\nYour transaction is successful\n\nAvailable Balance: {global_balance}\n\nThank you for using our', font=cour20, fg='blue')
    message.pack()
...
def message_func():
    ...
    message.pack()
    text = Label(win2, text='ATM', font=tim40, fg='orange')
    text.pack()

    exit_button = Button(win2, text='EXIT', font=cour15, fg='blue', command=lambda: win.destroy())
    exit_button.pack(side=BOTTOM, pady=10)
...
def option_func():
   ...
   # option_win.grab_set()                       ## check enter_pin() function for the functionality of .grab_set()
    ...
    withdrawal_btn = Button(rf, text=' WITHDRAWAL ', font=cour15, fg='pink', command=withdrawal_func)
...
intro = Label(win, text='\nWelcome to  virtual ATM', font=cour20, fg='green')
intro.pack()
